...
parser.add_argument('--metrics_Eastwood_dir', type=str, required=True)
parser.add_argument('--continuous_only', type=str2bool, default=True)
parser.add_argument('--LASSO_alpha', type=float, default=0.002)
parser.add_argument('--LASSO_iters', type=int, default=10000)
...
def main(args):
    ...
    result_file = join(args.metrics_Eastwood_dir, "{}_{}".format(args.enc_dec_model, args.run),
                       "results[LASSO,{},alpha={},iters={}].npz".format(
                            "cont" if args.continuous_only else "all",
                            args.LASSO_alpha, args.LASSO_iters))
    ...
    gt_factors = ['Scale', 'Rotation', 'Pos_x', 'Pos_y']

    R = np.abs(results['importance_matrix'])
    R_ids_sorted = np.argsort(R, axis=0)[::-1]
    R_sorted = np.take_along_axis(R, R_ids_sorted, axis=0)

    completeness_scores = results['completeness_scores']
    ...
    for n in range(len(data)):
        for k in range(len(gt_factors)):
            ...
            img_file = join(save_dir, "{}-x{}_LASSO_alpha={}].png".format(gt_factors[k], n, args.LASSO_alpha))

            ids_top3 = R_ids_sorted[:3, k]
            R_top3 = R_sorted[:3, k]
            model.cond_all_latents_traverse_v2(img_file, sess, data[n],
                                               z_comps=ids_top3,
                                               z_comp_labels=["z[{}] ({:.4f})".format(comp, mi)
                                                              for comp, mi in zip(ids_top3, R_top3)],
                                               span=span, points_1_side=points_one_side,
                                               hl_x=True,
                                               font_size=9,
                                               title="{} (Comp={:.4f})".format(
                                                   gt_factors[k], completeness_scores[k]),
