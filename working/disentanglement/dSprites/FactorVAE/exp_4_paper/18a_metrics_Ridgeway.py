...
from utils.metrics.metrics_ridgeway import compute_modularity, compute_explicitness
...
def main(args):
    ...
    print("\nCompute 'modularity'")
    results = compute_modularity(all_z_mean, y_train, is_discrete_z=False, is_discrete_y=True, num_bins=args.num_bins)

    print("\nCompute 'explicitness'")
    results_ = compute_explicitness(all_z_mean, y_train)
    results.update(results_)

    result_file = join(save_dir, "results[bins={}].npz".format(args.num_bins))
