...
def plot_curve():
    ...
    file_names = ['run_1Konny_0_default_summary_tf-tag-train_rec_x.csv',
                  'run_1Konny_1_tc50_multiSave_summary_tf-tag-train_rec_x.csv',
                  ]
    ...
    for n in range(len(ys)):
        ...
        if n == 1:
            ys[n] += 30
        plt.plot(common_x, ys[n][:min_common_len], label="{}".format(labels[n]), linewidth=2.0)
    ...
    pp = PdfPages(join(save_dir, 'rec_x_curve.pdf'))
    pp.savefig(plt.gcf())
    pp.close()
    plt.close()
