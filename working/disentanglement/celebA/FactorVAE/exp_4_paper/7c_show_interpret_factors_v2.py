...
def main(args):
    ...
    result_file = join(args.interpretability_metrics_dir, "FactorVAE_{}".format(args.run),
    ...
    for i in range(len(attr_names)):
        print("{}: JEMMIG: {:.4f}, JEMMIG (unnorm): {:.4f}, H_diff: {:.4f}, I2: {:.4f}, top 2 latents: z{}, z{}".format(
            attr_names[i], JEMMIG_norm[i], JEMMIG_unnorm[i], H_diff[i], MI_z_y_sorted[1, i],
    ...
    width = 0.3
    plt.bar(np.arange(len(attr_names)), JEMMIG_norm, width=width,
            align='center', edgecolor='black', label='JEMMIG')
    plt.bar(np.arange(len(attr_names)) + width, MI_gap_y, width=width,
            align='center', edgecolor='black', label='RMIG')

    plt.xticks(np.arange(len(attr_names)) + 0.5 * width, attr_names, rotation=90)
    plt.xlabel("model")
    ...
    '''
    # Sorted by y
    y_ids_sorted = np.argsort(MI_gap_y, axis=0)[::-1]
    sorted_attrs_names = [attr_names[idx] for idx in y_ids_sorted]

    MIG_sorted_by_y = np.take_along_axis(MI_gap_y, y_ids_sorted, axis=0)
